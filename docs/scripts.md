...
| `fm-teardown.sh`         | Return a clean, landed ship worktree or retire/release a secondmate home; requires scout reports, checks child work, removes firstmate-owned hook artifacts, archives a finished ship/scout task's `data/<id>/` folder into `data/archive/<project>/<id>/` on the success path, and prints the backend-aware backlog reminder |
